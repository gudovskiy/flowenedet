...
class CustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        # corruptions
        self.corrupt_typ = corrupt_typ
        self.corrupt_sev = corrupt_sev
        #
    ...
    def pre_eval(self, all_preds, indices):
    ...
    def evaluate(self,
        ...
        openIoU = True if 'oIoU' in metric else False
        eval_results = {}
        # test a list of files
        if mmcv.is_list_of(results, np.ndarray) or mmcv.is_list_of(results, str):
            if gt_seg_maps is None:
                gt_seg_maps = self.get_gt_seg_maps()  # efficient_test=True)
            #
            num_classes = len(self.CLASSES)
            num_classes = num_classes + 1 if openIoU else num_classes
            ret_metrics = eval_metrics(
                all_results,
                gt_seg_maps,
                num_classes,
                self.ood_class_index,
                self.ignore_index,
                metric,
                label_map=dict(),
                reduce_zero_label=self.reduce_zero_label)
        # test a list of pre_eval_results
            ...
            ret_metrics = pre_eval_to_metrics(all_results, metric)

        # Because dataset.CLASSES is required for per-eval.
        if self.CLASSES is None:
            class_names = tuple(range(num_classes))
        else:
            class_names = self.CLASSES + ('void',) if openIoU else self.CLASSES
