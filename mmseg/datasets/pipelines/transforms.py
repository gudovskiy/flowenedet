...
class Corruptions(object):
    ...
    def __init__(self, mode='test'):
        ...
        self.corruption_cats['all']     = ['motion_blur',    'defocus_blur',                'gaussian_blur',
    ...
    def __call__(self, results):
        ...
        if 'corrupt_sev' in results:
            if self.mode == 'test' and 'corrupt_sev' in results:
                typ = results['corrupt_typ']
                sev = results['corrupt_sev']
...
class AnomalyMosaic(object):
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Result dict with mosaic transformed.
        """
        # print(results.keys())  # {'img_metas', 'img', 'gt_semantic_seg', dict{'mix_results'}}
        # print(results['mix_results'][0].keys())  # {'img_metas', 'img', 'gt_semantic_seg'}
        # print(results['img'].data.shape)
        if np.random.uniform() < self.prob:
            results = self._anomaly_transform(results)
    ...
    def get_indexes(self, dataset):
        ...
        Returns:
            list: indexes.
        """
        # Cityscapes train datasset legnth (offset) = 2975!!!
        indexes = [random.randint(dataset.cumulative_sizes[0], len(dataset)) for _ in range(1)]
        return indexes

    def sample_shape(self, H, W):
        sizes = [i for i in range(16, max_dim, 8)]
        h = np.random.choice(sizes)
        w = np.random.choice(sizes)
        return h, w

    def _anomaly_transform(self, results):
        assert 'mix_results' in results
        ind_img = results['img'].data
        ind_seg = results['gt_semantic_seg'].data
        ano_img = results['mix_results'][0]['img'].data
        ano_seg = results['mix_results'][0]['gt_semantic_seg'].data
        #
        ind_msk = torch.zeros_like(ind_seg)  # 1xHxW
        ano_msk = torch.zeros_like(ano_seg)  # 1xHxW
        #  sample random mask
        OH, OW = ind_seg.shape[-2:]
        AH, AW = ano_seg.shape[-2:]
        if np.random.uniform() < self.rect_ood_prob:
            h = np.random.choice([i for i in range(0, AH, 32)])
            w = np.random.choice([i for i in range(0, AW, 32)])
            # patch size
            ch = np.random.randint(0,AH-h)
            cw = np.random.randint(0,AW-w)
            ano_msk[:, ch:ch+h, cw:cw+w] = 1
        else:
            indices = np.random.choice(ano_seg.view(-1).unique().numpy(), self.ood_classes_per_item)
            for idx in indices:
                ano_msk[ano_seg == idx] = 1
        # paste random anomaly assuming that ano_seg dims are divisible of ind_seg
        pi = np.random.randint(0,OH//AH)
        pj = np.random.randint(0,OW//AH)
        ind_msk[:, pi*AH:(pi+1)*AH, pj*AW:(pj+1)*AW] = ano_msk
        #print(torch.sum(ind_msk), torch.sum(ano_msk))
        ind_img[:, ind_msk.squeeze(0)==1] = ano_img[:, ano_msk.squeeze(0)==1]
        ind_seg[ind_msk==1] = self.anomaly_index
        #results['img'].data = ind_img
        #results['gt_semantic_seg'].data = ind_seg
        #print(pi,ph,pj,pw,results['gt_semantic_seg'].data[:, pi:pi+ph, pj:pj+pw])
        #print(torch.sum(results['img'].data[:, ind_msk.squeeze(0)==1]-ano_img[:, ano_msk.squeeze(0)==1]))
        return results
